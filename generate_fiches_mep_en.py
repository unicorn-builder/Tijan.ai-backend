"""
generate_fiches_mep_en.py — MEP Equipment Technical Sheets
Tijan AI — Electrical, Plumbing, HVAC, Automation
Dakar suppliers included
"""
...
def hf(canvas,doc,nom,date_str):
    ...
    canvas.drawString(15*mm,h-17*mm,f"{nom}  —  MEP Equipment Technical Sheets & Automation")
...
def fiche(titre, specs, fournisseur, note=None):
    ...
    story.append(t)
    story.append(Paragraph(f"Dakar Suppliers : {fournisseur}", 
...
def generer_fiches_mep(resultats, buf, params_dict=None):
    ...
    doc = SimpleDocTemplate(buf,pagesize=A4,rightMargin=15*mm,leftMargin=15*mm,
        topMargin=25*mm,bottomMargin=20*mm,title=f"MEP Sheets — {nom}",author="Tijan AI")
    ...
    story.append(Paragraph("Electrical | Plumbing | HVAC | Automation — Dakar Suppliers",st['sub']))
    ...
    story += fiche(f"EL-02 Groupe Electrogene Secours {int(P_total*0.65/100)*100} kVA",
        [("Puissance de secours",f"{int(P_total*0.65/100)*100} kVA"),("Moteur diesel","6 cylindres en ligne"),
         ("Regime nominal","1 500 tr/min (50 Hz)"),("Noise level","< 75 dB(A) a 7m"),
    ...
    story.append(Paragraph("2. PLUMBING EQUIPMENT",st['h2']))
    story.append(HRFlowable(width="100%",thickness=0.5,color=GRIS_CLAIR))

    story += fiche("PL-01 Surpresseur Eau Domestique — 6 m³/h",
        [("Nominal flow rate","6 m³/h"),("Head pressure","70 mCE"),
    ...
    story += fiche("PL-02 Solar water heater 200L with electric backup",
        [("Volume ballon","200 L"),("Surface capteur","2 x 2.0 m² (plat selectif)"),
         ("Collector thermal power","2 600 W/capteur"),("Electric backup","3 000 W — stainless steel sheathed element"),
         ("Temperature maximale","90°C (protection thermique)"),("Isolation ballon","Polyurethane 50mm"),
         ("Pression maxi service","8 bar"),("Efficiency global","> 70% (conditions Dakar)"),
    ...
    # 3. HVAC EQUIPMENT
    story.append(Paragraph("3. HVAC EQUIPMENT",st['h2']))
    story.append(HRFlowable(width="100%",thickness=0.5,color=GRIS_CLAIR))

    story += fiche("CV-01 Split Inverter Mural — Sejour 18 000 BTU/h",
        [("Cooling capacity","18 000 BTU/h (5.3 kW)"),("Heating capacity","20 000 BTU/h (5.9 kW)"),
         ("COP chauffage","4.0"),("EER refroidissement","3.6"),
         ("Energy class","A++ (refroidissement)"),("Fluide frigorigene","R32 — GWP 675"),
         ("Temperature ext. maxi","52°C (adapte Dakar)"),("Noise level unite int.","< 35 dB(A)"),
    ...
    story += fiche("CV-02 VMC Double Flux — 150 m³/h par logement",
        [("Nominal flow rate","150 m³/h (reglable 80-200 m³/h)"),("Efficacite enthalpique",">= 85%"),
         ("Electrical power","60 W (basse vitesse) / 120 W (haute)"),("Noise level","< 28 dB(A) a 3m"),
    ...
    # 4. BUILDING AUTOMATION BMS
    story.append(Paragraph("4. EQUIPEMENTS BUILDING AUTOMATION & BMS",st['h2']))
    story.append(HRFlowable(width="100%",thickness=0.5,color=GRIS_CLAIR))
    ...
    story.append(Paragraph(
        "Technical sheets for reference only. Final specifications to be validated by the responsible MEP engineer. "
