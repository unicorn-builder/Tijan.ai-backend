...
Format BPU: No. | Description | Unit | Qty | Basic | High-End | Luxury
...
def _row(lot, desig, unite, qte, pu_b, pu_h, pu_l, bold=False):
    """Build a 7-column row: No. | Description | Unit | Qty | Basic | High-End | Luxury"""
...
def _build(rm):
    ...
    story.append(Paragraph(
        ...
        S['body']))
    story.append(Paragraph(
        'Unit prices local market 2026 — supplied and installed. Margin ±15%. '
    ...
    rows_plomb = [
        _row('P.1', f'Polyethylene water tank {int(pl.volume_citerne_m3)} m³', 'ea',
             nb_cuves, cuve_pu, int(cuve_pu*1.20), int(cuve_pu*1.50)),
        _row('P.2', f'Booster pump {pl.debit_surpresseur_m3h} m³/h — with VSD', 'ea',
             1, surp_pu, int(surp_pu*1.30), int(surp_pu*1.70)),
        _row('P.3', f'Cold water risers DN{pl.diam_colonne_montante_mm}', 'lm',
    ...
    if pl.nb_chauffe_eau_solaire > 0:
        rows_plomb.append(_row('P.9', f'Solar water heaters 200L — {pl.nb_chauffe_eau_solaire} u.', 'ea',
    ...
    if cv.nb_splits_sejour > 0:
        rows_cvc += [
            _row('C.1', f'Wall mounted splits living area 1HP — {cv.nb_splits_sejour} u.', 'ea',
                 cv.nb_splits_sejour, px.split_1cv, int(px.split_1cv*1.35), int(px.split_1cv*1.80)),
            _row('C.2', f'Wall mounted splits bedroom 1HP — {cv.nb_splits_chambre} u.', 'ea',
                 cv.nb_splits_chambre, px.split_1cv, int(px.split_1cv*1.35), int(px.split_1cv*1.80)),
        ]
    if cv.nb_cassettes > 0:
        rows_cvc.append(_row('C.1', f'Ceiling mounted cassettes 4HP — {cv.nb_cassettes} u.', 'ea',
    ...
    rows_cvc += [
        _row('C.3', f'Ventilation unit {cv.type_vmc} — {cv.nb_vmc} u.', 'ea',
             cv.nb_vmc, vmc_pu, int(vmc_pu*1.25), int(vmc_pu*1.60)),
        _row('C.4', 'Ventilation ductwork network', 'm²',
             int(surf*0.08), 18_000, 25_000, 35_000),
        _row('C.5', 'Grilles + diffusers', 'ea',
             int(rm.nb_logements * 4), 15_000, 25_000, 45_000),
        _row('C.6', 'Zone control + thermostat', 'zone',
    ...
    if si.sprinklers_requis and si.nb_tetes_sprinkler > 0:
        rows_si += [
            _row('SI.8', f'Sprinkler system + supply network', 'ea',
                 1, px.sprinkler_centrale, int(px.sprinkler_centrale*1.2), int(px.sprinkler_centrale*1.5)),
            _row('SI.9', f'Sprinkler heads — {si.nb_tetes_sprinkler} u.', 'ea',
    ...
    if si.desenfumage_requis:
        ...
        rows_si.append(_row('SI.10', 'Smoke evacuation — motorized dampers + ducts', 'lump sum',
    ...
    if asc.nb_ascenseurs > 0:
        ...
        rows_asc = [
            _row('ASC.1', f'Elevator {asc.capacite_kg}kg {asc.vitesse_ms}m/s — {asc.nb_ascenseurs} u.', 'ea',
                 asc.nb_ascenseurs, asc_pu, int(asc_pu*1.15), int(asc_pu*1.40)),
            _row('ASC.2', 'RC hoistway + pit', 'ea',
                 asc.nb_ascenseurs, 3_500_000, 4_200_000, 4_200_000),
            _row('ASC.3', 'Dedicated electrical panel for elevators', 'ea',
                 asc.nb_ascenseurs, 850_000, 1_200_000, 1_500_000),
        ]
        if asc.nb_monte_charges > 0:
            rows_asc.append(_row('ASC.4', f'Service lift 500kg — {asc.nb_monte_charges} u.', 'ea',
    ...
    rows_auto = [
        _row('AUTO.1', f'BMS/GTB system — {auto.protocole}', 'lump sum', 1,
        ...
        _row('AUTO.2', f'Lighting management — {auto.nb_points_controle} points', 'point',
        ...
        _row('AUTO.3', 'Centralized HVAC management', 'lump sum', 1,
             2_500_000, 4_500_000, 8_000_000),
    ]
    if auto.gestion_energie:
        rows_auto.append(_row('AUTO.4', 'Energy metering + dashboards', 'lump sum', 1,
             3_500_000, 6_000_000, 10_000_000))
    if not auto.bms_requis:
        rows_auto.append(_row('AUTO.5', f'Home automation per unit — {rm.nb_logements} u.', 'ea',
    ...
    recap_rows = [[p(h,'th') for h in ['Lot','Description','BASIC','HIGH-END','LUXURY','%']]]
