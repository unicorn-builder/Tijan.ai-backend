...
Format BPU : N° | Désignation | Unité | Quantité | PU | Montant | Observations
"""
import io, math
...
def _build(rs):
    ...
    story.append(Paragraph(
        f'Bordereau des Prix Unitaires — Structure — {d.ville} {rs.params.pays}',
        S['sous_titre']))
    story.append(Paragraph(
    ...
    rows_inst = [
        _row('1.1', 'Clôture de chantier (palissade bois ou tôle)', 'ml', q_cloture, 15000, m_cloture),
        _row('1.2', 'Base vie chantier (bureau, vestiaires, sanitaires)', 'forfait', 1, m_base, m_base, 'Modulaires démontables'),
        _row('1.3', 'Branchements provisoires eau + électricité', 'forfait', 1, m_branch, m_branch),
        _row('1.4', 'Signalétique sécurité + EPI chantier', 'forfait', 1, m_secu, m_secu),
        _row('1.5', 'Repli et nettoyage fin de chantier', 'forfait', 1, m_repli, m_repli),
    ...
    rows_terr = [
        _row('2.1', 'Décapage terre végétale e=30cm', 'm³', int(V_decap), px.terr_mecanique_m3, m_decap),
        _row('2.2', 'Fouilles générales mécaniques', 'm³', int(V_fouilles), px.terr_mecanique_m3, m_fouilles, 'Engins mécaniques'),
        _row('2.3', 'Fouilles manuelles en fond de fouille', 'm³', q_fouilles_man, px.terr_manuel_m3, m_fouilles_man, 'Finitions manuelles'),
        _row('2.4', 'Remblai compacté (matériaux sélectionnés)', 'm³', int(V_remblai), px.remblai_m3, m_remblai),
        _row('2.5', 'Évacuation terres excédentaires', 'm³', int(V_evacu), 5000, m_evacu, 'Transport + décharge agréée'),
        _row('2.6', 'Lit de sable sous dallage e=10cm', 'm³', int(V_sable), 12000, m_sable),
        _sous_total('SOUS-TOTAL LOT 2', c_terr_all),
    ]
    story.append(make_table(rows_terr))
    recap['lot2'] = c_terr_all
    ...
    if fond.nb_pieux > 0:
        ...
        rows_fond = [
            _row('3.1', f'Pieux forés béton armé Ø{fond.diam_pieu_mm}mm — L={fond.longueur_pieu_m}m', 'ml', q_pieux_ml, prix_pieu, c_pieux, f'{fond.nb_pieux} pieux × {fond.longueur_pieu_m}m'),
            _row('3.2', f'Armatures pieux HA500B — cage Ø{fond.diam_pieu_mm}mm', 'kg', int(kg_fond_acier), prix_acier, m_acier_fond, f'As={fond.As_cm2}cm² par pieu'),
            _row('3.3', f'Béton de propreté e=10cm sous longrines', 'm³', max(1, q_proprete), _pu(m_proprete, max(1, q_proprete)), m_proprete),
            _row('3.4', f'Longrines béton armé {rs.classe_beton} — section 30×50cm', 'ml', q_longr, 85000, c_longr, 'Liaisons entre pieux'),
            _row('3.5', 'Tête de pieux + chapiteaux béton armé', 'U', nb_pot, 120000, m_tete, 'Connexion pieux-structure'),
            _sous_total('SOUS-TOTAL LOT 3', c_fond),
        ]
        story.append(Paragraph(
            f'Les fondations représentent {c_fond/boq.total_bas_fcfa*100:.0f}% du budget structure — '
        ...
        rows_fond = [
            _row('3.1', 'Béton de propreté e=10cm', 'm³', max(1, q_proprete2), _pu(m_proprete2, max(1, q_proprete2)), m_proprete2),
            _row('3.2', f'Semelles béton armé {rs.classe_beton}', 'm³', max(1, q_sem_bet), prix_beton, m_semelle_bet),
            _row('3.3', f'Armatures semelles {rs.classe_acier}', 'kg', q_sem_acier, prix_acier, m_semelle_acier),
            _row('3.4', 'Coffrage semelles perdues', 'm²', max(1, q_sem_coff), px.coffrage_bois_m2, m_semelle_coff),
            _sous_total('SOUS-TOTAL LOT 3', c_fond),
        ]
    ...
    rows_pout = [
        HEADERS,
        _row('4.2.1', f'Poutres principales {poutre.b_mm}×{poutre.h_mm}mm — portée max {d.portee_max_m}m', 'ml', int(L_px), _pu(m_pout_princ, int(L_px)), m_pout_princ, f'As inf={poutre.As_inf_cm2}cm² / As sup={poutre.As_sup_cm2}cm²'),
        _row('4.2.2', f'Poutres secondaires {poutre.b_mm}×{poutre.h_mm}mm — portée min {d.portee_min_m}m', 'ml', int(L_py), _pu(m_pout_sec, int(L_py)), m_pout_sec),
        _row('4.2.3', f'Armatures poutres {rs.classe_acier} (façonnées + posées)', 'kg', int(kg_pout), prix_acier, c_pout_acier, f'Étriers HA{poutre.etrier_diam_mm} e={poutre.etrier_esp_mm}mm'),
        _row('4.2.4', 'Coffrage poutres (bois + étais)', 'm²', q_pout_coff, _pu(c_pout_coff, q_pout_coff), c_pout_coff),
    ...
    rows_dalles = [
        HEADERS,
        _row('4.3.1', f'Béton dalles {rs.classe_beton} BPE e={dalle.epaisseur_mm}mm', 'm³', int(V_dalles), prix_beton, c_dalle_beton, f'As_x={dalle.As_x_cm2_ml}cm²/ml / As_y={dalle.As_y_cm2_ml}cm²/ml'),
        _row('4.3.2', f'Armatures dalles {rs.classe_acier} — HA10 deux sens', 'kg', int(kg_dalles), prix_acier, c_dalle_acier, 'Treillis soudé + renforts'),
        _row('4.3.3', 'Coffrage dalles (banches métalliques)', 'm²', q_dalle_coff, px.coffrage_bois_m2, c_dalle_coff),
        _row('4.3.4', 'Étaiement provisoire (échafaudages)', 'm²', q_dalle_coff, 3500, m_etai, 'Location + main d\'oeuvre'),
        _row('4.3.5', 'Réservations trémies (gaines, escaliers)', 'forfait', 1, m_reserv, m_reserv, 'Estimation 0.5% SHOB'),
    ...
    rows_esc = [
        HEADERS,
        _row('4.4.1', f'Escaliers béton armé {rs.classe_beton} — volée + paliers', 'volée', q_esc, 450000, c_esc, f'{nb_esc} cage(s) × {d.nb_niveaux} niveaux'),
        _row('4.4.2', 'Garde-corps béton/métal escaliers', 'ml', q_garde, 45000, m_garde),
        _sous_total('Sous-total escaliers', c_esc_total),
    ]
    ...
    rows_maco.append(_row('5.1', 'Maçonnerie façades extérieures — agglos pleins 25cm',
        'm²', q_fac, px.agglo_plein_25_m2, c_fac, 'Murs périphériques enduits 2 faces'))
    c_maco_total += c_fac
    ...
    rows_maco.append(_row('5.3', 'Cloisons légères intérieures — BA13 simple rail',
        'm²', q_leg, px.ba13_simple_m2, c_leg, 'Cloisons intérieures non séparatives'))
    c_maco_total += c_leg
    ...
    rows_maco.append(_row('5.5', 'Enduits ciment + plâtre intérieur/extérieur',
        'm²', surf_enduit, 8500, c_end, 'Finitions surfaces maçonnées'))
    c_maco_total += c_end
    ...
    story.append(Paragraph(
        'Plusieurs options de cloisons disponibles selon budget et usage. '
    ...
    rows_etanch = [
        HEADERS,
        _row('6.1', 'Étanchéité toiture-terrasse SBS bicouche (primaire + 2 couches)', 'm²', surf_etanch, px.etanch_sbs_m2, c_sbs, 'Soudage à la flamme — DTU 43.1'),
        _row('6.2', 'Protection étanchéité — chape de protection e=5cm', 'm²', surf_etanch, 5000, c_prot),
        _row('6.3', 'Dallettes béton sur plots (toiture accessible)', 'm²', q_dallettes, 8000, c_dallettes, '40% toiture accessible'),
        _row('6.4', 'Étanchéité salles de bains + locaux humides', 'm²', q_salles, px.etanch_liquide_m2, c_salles, 'Résine monocomposant'),
    ]
    if d.avec_sous_sol:
        q_ss = int(d.surface_emprise_m2)
        rows_etanch.append(_row('6.5', 'Étanchéité sous-sol (membrane PVC)', 'm²', q_ss, int(px.etanch_pvc_m2*0.30), c_ss_sol, 'Cuvelage intérieur'))
    ...
    sous_tot_travaux = (inst_forfait + c_terr_all + c_fond + c_lot4 +
    ...
    rows_div = [
        HEADERS,
        _row('7.1', 'Joints de dilatation (tous niveaux)', 'U', max(1, q_joints), _pu(m_joints, max(1, q_joints)), m_joints, '1 joint / 500m² environ'),
        _row('7.2', 'Réservations et scellements divers', 'forfait', 1, m_reserv7, m_reserv7, 'Gaines électriques, plomberie'),
        _row('7.3', 'Imprévus chantier (5% travaux)', 'forfait', 1, c_impr, c_impr, 'Aléas et adaptations'),
        _row('7.4', 'Honoraires BET structure (4% travaux)', 'forfait', 1, c_bet, c_bet, 'Plans d\'exécution + suivi'),
        _row('7.5', 'Contrôle technique (1.5% travaux)', 'forfait', 1, c_ctrl, c_ctrl, 'Organisme agréé obligatoire'),
        _sous_total('SOUS-TOTAL LOT 7', c_div_total),
    ]
    ...
    lot_data = [
        ('1', 'Installation de chantier', recap.get('lot1', 0)),
        ('2', 'Terrassement', recap.get('lot2', 0)),
        ('3', 'Fondations', recap.get('lot3', 0)),
        ('4', 'Structure béton armé (poteaux + poutres + dalles + escaliers)', recap.get('lot4', 0)),
        ('5', 'Maçonnerie et cloisons', recap.get('lot5', 0)),
        ('6', 'Étanchéité et isolation', recap.get('lot6', 0)),
        ('7', 'Divers + honoraires techniques', recap.get('lot7', 0)),
    ]

    recap_rows = [
        [p('LOT','th'), p('DÉSIGNATION','th_l'),
         p(f'MONTANT ({dl})','th'), p('% TOTAL','th')],
    ]
